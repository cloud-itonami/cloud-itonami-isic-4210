(ns roadrail.notify
  ...
  road/railway domain analog of `construction.notify`/`demolition.
  notify`. Only this namespace touches the network (java.net.http,
  Resend, Twilio) or the RESEND_API_KEY/TWILIO_* env vars; every other
  namespace in this actor stays pure per its own contract.
  ...
  `:safety-contacts` roster (the licensed civil engineer / site
  ...
  (see `roadrail.phase`/`roadrail.governor` ns docstrings), so by the
  time `roadrail.operation`'s `:commit` node calls this, a human has
...
(defn dispatch-safety-concern-notice!
  ...
  every other responsible party of a flagged structural/traffic-control/
  utility-strike concern."
