(ns roadrail.operation
  "OperationActor -- one road/railway-construction-project coordination
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (Road-Rail Advisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Road-Rail
  ...
  One graph run = one road/railway-construction coordination operation
  ...
  decision to a human operator (the licensed civil engineer / site
  supervisor). The approver resumes with `{:approval {:status :approved}}`
  (or :rejected). `:schedule-construction-operation`/`:flag-safety-
  concern` ALWAYS reach this node -- see `roadrail.phase`/`roadrail.
  ...
  is clean -- see `roadrail.phase` ns docstring.

  Every committed record carries `:effect :propose` (see `roadrail.
  ...
  ADDITIONALLY send the safety-concern notice (mail + phone, `roadrail.
...
(defn build
  ...
  `roadrail.store/Store`).
  opts:
    :advisor      -- a `roadrail.advisor/Advisor` (default: mock-advisor)
    :notifier     -- a `roadrail.notify/Notifier` (default: mock-notifier)
  ...
  (-> (g/state-graph
      ...
      ;; Road-Rail Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Road-Rail Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
