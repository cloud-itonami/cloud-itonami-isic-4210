(ns roadrail.store-contract-test
  ...
  contract-test`/`demolition.store-contract-test` for the same pattern on
  the sibling actors."
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      (is (= "Sakura Avenue Road Widening" (:name (store/site s "site-1"))))
    ...
    (testing label
      (testing "log-site-record commits a patch and appends a site-record-log entry"
        (store/commit-record! s {:op :log-site-record :path ["site-1"]
                                 :value {:id "site-1" :grading-percent-complete 75}})
        (is (= 75 (:grading-percent-complete (store/site s "site-1"))))
        (is (= "Sakura Avenue Road Widening" (:name (store/site s "site-1"))) "unrelated field preserved")
      ...
      (testing "sequences are jurisdiction-scoped, not global"
        (store/commit-record! s {:op :log-site-record :path ["site-7"]
                                 :value {:id "site-7" :grading-percent-complete 10}})
